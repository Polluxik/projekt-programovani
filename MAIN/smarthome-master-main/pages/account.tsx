...
const getData = async () => {
    const doc = await fire.firestore().collection("listusers").doc(fire.auth().currentUser?.uid).get();
    const data = doc.data();
    const dataarray = (doc.data()) || "")['watched'];
}
    ...
    return (
        <main className={styles.body}>
            <div>
                ...
                <p>Your watched: {getData}</p>
                <button onClick={getData} >Change Password</button>
                <div>
                    <p></p>
                </div>
            </div>
